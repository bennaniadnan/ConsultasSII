CREATE PROCEDURE [dbo].[OPE_GET_LATEST_OPERATIONS_BY_FILTER_START_RECORDS]
(
	@IdUsuario VARCHAR(6),
	@Id int = null,
	@Ejercicio int = null,
...
BEGIN
	...
	--	INNER JOIN RegistroInformacion RI with (nolock) ON RO.IdRegistroInformacion = RI.Id
		...
		where --@IdUsuario = op.IdUsuario AND 
			ISNULL(OP.Id, '') = ISNULL(@Id, ISNULL(OP.Id,''))
		...
		ORDER BY OP.Id desc, OP.FechaOperacion desc, OP.HoraOperacion desc OFFSET @Start ROWS FETCH NEXT @Records ROWS ONLY
