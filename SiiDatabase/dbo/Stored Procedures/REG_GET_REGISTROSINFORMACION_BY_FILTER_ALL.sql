CREATE PROCEDURE [dbo].[REG_GET_REGISTROSINFORMACION_BY_FILTER_ALL]
(
...
AS
BEGIN
select * into #tmp_RegistrosOperacion from RegistrosOperacion ro
--inner join RegistrosOperacion ro on ro.IdOperacion = o.Id
--inner join RegistroInformacion r on r.id = ro.IdRegistroInformacion
where ro.IdOperacion = @IdOperacion
--select * from #tmp_RegistrosOperacion
                ...
                SELECT  --TOP(@top)
                  ...
                  FROM [dbo].[RegistroInformacion] RI with (nolock)
                  INNER JOIN
                  #tmp_RegistrosOperacion RO with (nolock)
                    ON RO.IdRegistroInformacion = RI.Id and RO.IdOperacion = isnull(@IdOperacion,RO.IdOperacion)
                  INNER JOIN
...
drop table #tmp_RegistrosOperacion
